...
def make_sigsdf(dfchain, llhoods, dnames, resultspath):
	# taken from multimoon mm_plots_multi.py
	...
	#llhoods = sampler.get_log_prob(discard=int(burnin+clusterburn),flat = True, thin=thin_plots) <- used in multimoon code
	# I don't think this code needs llhoods, so I'm leaving it out
	ind = np.argmax(llhoods)
