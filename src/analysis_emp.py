...
def plots(sampler, resultspath, runprops):
	...
	# Change the f indes from 2 to 0 for non HST data
	...
	if npsfs == 1:
		...
		# Including ra and dec into our chain for the sigsdf file (empirical addition)
		num = 1            
		for i in range(npsfs):   
			dnames = np.append(dnames,"ra"+ str(num))
			dnames = np.append(dnames,"dec"+ str(num))
			num += 1
		...
		num = 1            
		for i in range(npsfs):   
			dfchain = np.concatenate((dfchain,np.array(astromdf['ra'+ str(num)]).reshape((astromdf['ra'+ str(num)].size,1)) ), axis = 1)
			dfchain = np.concatenate((dfchain,np.array(astromdf['dec'+ str(num)]).reshape((astromdf['dec'+ str(num)].size,1)) ), axis = 1)
			num += 1            
		...
		make_sigsdf_emp(dfchain, llhoods, dnames, npsfs, resultspath)
       
	else:
		print("Error, enter a whole number of objects, 1 or greater. Aborting analysis.")      
        
def make_sigsdf_emp(dfchain, llhoods, dnames, npsfs, resultspath):        
...
def make_obsdf(dfchain, astromdf, f, objectnames, npsfs, resultspath):
	...
	# Input a hardcoded time
	timeJD = f[0].header['MJD-OBS']
